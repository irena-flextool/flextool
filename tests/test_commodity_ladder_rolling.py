...
def _read_objective(workdir: Path) -> float:
    ...
    Inlined from the retired tests/test_commodity_ladder_smoke.py
    (Tier 4 Commit 8 — flextoolrunner retirement); only used by the
    single-solve bit-identity check below.
    """
...
def _run(
    scenario: str, db_url: str, solver_config_dir: Path, workdir: Path,
) -> None:
...
class TestCrossPeriodRolling:
    ...
    def test_end_to_end_and_accumulator_files_present(
        ...
        _run("coal_cum_rolling", cumulative_db_url_underspend,
             test_solver_config_dir, workdir)
...
class TestSingleSolveBitIdentity:
    ...
    def test_single_solve_cumulative_matches_coal_objective(
        ...
        os.chdir(coal_dir)
        _run("coal", test_db_url, test_solver_config_dir, coal_dir)
        ...
        _run("coal_cum_single", cumulative_db_url_underspend,
             test_solver_config_dir, cum_dir)
