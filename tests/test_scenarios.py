...
def test_scenario(
    ...
    # ``flextool/engine_polars/_solver_base.py``).  ``run_chain_from_db``
    ...
    write_outputs(
        ...
        solve_steps=[
            # ``effective_solution`` returns the live polar-high
            # Solution when its ``_vars`` is still populated, else a
            # :class:`SnapshotSolution` over the per-sub-solve captured
            # decision-variable frames.  Polar-high releases earlier
            # sub-solves' ``_vars`` internally between sub-solves
            # (memory optimisation on ``polar-high-opt``), so the
            # snapshot is needed for ``_entity_all_capacity`` to see
            # each roll's own ``v_invest`` / ``v_divest``.
