...
@pytest.fixture(scope="session")
def stoch_two_period_db_url(tmp_path_factory: pytest.TempPathFactory) -> str:
    """``stoch_two_period`` fixture DB — multi-period stochastic
    continuation fan-out (design
    ``specs/step05_continuation_fanout_design.md`` §5.1).

    Built from ``tests/fixtures/stoch_two_period.json`` (generated by
    ``tests/fixtures/build_stoch_two_period.py``).  Defensive migration
    as in :func:`stochastic_db_url`.
    """
    from flextool.update_flextool.db_migration import migrate_database

    db_path = tmp_path_factory.mktemp("db_stp") / "stoch_two_period.sqlite"
    url = json_to_db(FIXTURES_DIR / "stoch_two_period.json", db_path)
    migrate_database(url)
    return url
...
@pytest.fixture(scope="session")
def stoch_two_period_invest_na_db_url(
    tmp_path_factory: pytest.TempPathFactory,
) -> str:
    """``stoch_two_period_invest_na`` fixture DB — Slice D variant B
    (cross-scenario coupling; design §15.6).

    Built from ``tests/fixtures/stoch_two_period_invest_na.json``
    (generated by ``tests/fixtures/build_stoch_two_period_invest_na.py``):
    the proven ``stochastics.json`` storage shape extended with a
    ``recourse_na`` scenario that makes the stochastic-group storage node
    investable under ``recourse``.  Defensive migration as in
    :func:`stochastic_db_url`.
    """
    from flextool.update_flextool.db_migration import migrate_database

    db_path = (tmp_path_factory.mktemp("db_stpina")
               / "stoch_two_period_invest_na.sqlite")
    url = json_to_db(
        FIXTURES_DIR / "stoch_two_period_invest_na.json", db_path)
    migrate_database(url)
    return url
