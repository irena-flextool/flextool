"""flexpy ``capacity_margin`` parity — exercises the
``capacityMargin`` constraint and ``vq_capacity_margin`` slack from
the ``_group_slack`` feature module."""
...
def test_capacity_margin_parity():
