...
polar_high_spike is a *consumer* of those CSVs (preprocessing happens
upstream in flextool), so the equivalent invariants in polar_high are
on-disk shape contracts the input/model layer relies on.

This test pins the two block-aware paths polar_high actually walks:
...
an upstream regression in the block-CSV writer (or in polar_high's
...
def test_block_preprocessing_invariants_single_block(scenario_workdir):
    """Single-block degeneracy on ``work_base``."""
    work = scenario_workdir("base")
    _check_single_block_degeneracy(work / "solve_data")


def test_block_preprocessing_invariants_multi_block(scenario_workdir):
    """Multi-block aggregation on the ``lh2_three_region`` scenario."""
    work = scenario_workdir("lh2_three_region", db_fixture="lh2")
    _check_multi_block_aggregation(work / "solve_data")
