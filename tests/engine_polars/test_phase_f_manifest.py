...
def _copy_fixture(tmp_path: Path) -> Path:
    """Copy the ``work_base`` fixture into ``tmp_path`` so the test
    doesn't pollute the source-controlled fixture with a manifest.json."""
...
def test_manifest_has_documented_schema(tmp_path: Path) -> None:
...
def test_manifest_entries_match_registry(tmp_path: Path) -> None:
...
def test_manifest_present_files_exist_on_disk(tmp_path: Path) -> None:
...
def test_registry_columns_match_parquet_schema(
    tmp_path: Path, registry_key: str,
) -> None:
