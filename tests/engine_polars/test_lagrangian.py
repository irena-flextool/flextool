...
WORK_LH2 = Path(__file__).resolve().parent / "data" / "work_lh2_three_region"
...
def test_lh2_three_regions_solved(lh2_data) -> None:
    """All three regions should be solved and reported in
    region_objectives."""
    result = solve_lagrangian(
        lh2_data, work_dir=WORK_LH2, alpha=1.0, max_iters=20, tol=1.0,
    )
...
def test_lh2_iteration_log_populated(lh2_data) -> None:
    result = solve_lagrangian(
        lh2_data, work_dir=WORK_LH2, alpha=1.0, max_iters=10, tol=1.0,
        min_iters=10,
    )
...
class TestConvergenceBehavior:
    def test_max_iters_returns_unconverged(self, lh2_data) -> None:
        """When max_iters is too small to converge, return with
        converged=False."""
        result = solve_lagrangian(
            lh2_data, work_dir=WORK_LH2, alpha=0.001, max_iters=3, tol=1e-9,
            min_iters=3,
        )
...
class TestErrorPaths:
    ...
    def test_non_lagrangian_scenario_errors(self) -> None:
        """A scenario without decomposition_method should raise."""
        # work_base has no decomposition_method param.
        WORK_BASE = Path(__file__).resolve().parent / "data" / "work_base"
...
def test_primal_recovery_total_recorded(lh2_data) -> None:
    """The recovery solve appends a 'best_dual_total' / 'recovered_total'
    log entry so callers can see both bounds."""
    result = solve_lagrangian(
        lh2_data, work_dir=WORK_LH2, alpha=1.0, max_iters=30, tol=1.0,
        min_iters=30,
    )
