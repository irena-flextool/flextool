...
into ``polar_high_spike``.  The functional half of the flextool test
...
polar_high's RP machinery surfaces this through the ``op_factor`` term in
...
0.5 vs. 2.5.  This test re-solves both with polar_high and asserts the
...
SCEN_HALF = "years_represented_half"
SCEN_2_5 = "years_represented_2_5"
...
def test_rp_total_cost_equals_sum_of_rp_cost_times_rp_weight(scenario_workdir):
    ...
    """
    work_half = scenario_workdir(SCEN_HALF)
    work_2_5 = scenario_workdir(SCEN_2_5)
    ...
    # 3) polar_high matches flextool's published objective on each fixture
    #    (closed-form RP weighting is what flextool also uses).
    for work, polar_high_obj in ((work_half, obj_half), (work_2_5, obj_2_5)):
        ft_obj = _flextool_published_obj(work)
        rel_p = abs(polar_high_obj - ft_obj) / max(1.0, abs(ft_obj))
        assert rel_p < 1e-6, (
            f"{work.name}: polar_high={polar_high_obj} vs flextool={ft_obj}, rel={rel_p}"
        )
