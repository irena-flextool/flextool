...
the matching multiplier landed in ``flextool/model.py``'s
``conversion_indirect`` emission via the
``p_process_sink_conversion_flow_coeff`` Param plumbed through
``_load_indirect``.
"""
...
SCENARIO = "coal_chp_extraction"

def test_coal_chp_extraction_parity(scenario_workdir):
    ...
    assert data.p_process_sink_conversion_flow_coeff is not None
    pb = Problem()
