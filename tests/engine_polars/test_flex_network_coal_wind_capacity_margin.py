"""flexpy ``network_coal_wind_capacity_margin`` parity — exercises
``capacityMargin`` on a multi-node fixture with both unit and
...
WORK = Path(__file__).resolve().parent / "data" / "work_network_coal_wind_capacity_margin"
...
def test_network_coal_wind_capacity_margin_parity():
