"""polar_high port of flextool's ``test_timeset_weights.py``.
...
guidance "polar_high's loader already handles the inputs but should
...
class TestLoadRpCostWeight:
    ...
    def test_missing_steps_default_to_one(self, tmp_path: Path) -> None:
        ...
        ``test_missing_steps_are_treated_as_zero``.  polar_high's loader has
        ...
        would have produced explicit 0.0 / 2.0 entries; polar_high's loader
...
def test_real_fixture_has_writer_invariant(scenario_workdir) -> None:
    """End-to-end: ``work_base_weighted`` is the canonical
    real-fixture flextool emits with non-uniform weights.  Verify the
    on-disk file polar_high consumes already encodes the writer's
