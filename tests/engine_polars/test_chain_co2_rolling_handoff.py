...
between solves — but through flexpy's ``run_chain`` +
``build_handoff_from_flexpy`` instead of flextool's CSV writer.

flexpy's :class:`SolveHandoff` exposes the field ``cumulative_co2``
for this carrier.  The current flexpy build pipeline does NOT
...
``build_handoff_from_flexpy`` leaves the slot at ``None``.
...
When flexpy gains support for cumulative CO2 (Phase-2 work
...
def test_chain_cumulative_co2_handoff_diagnostic() -> None:
    ...
    1. ``handoff.cumulative_co2`` is ``None`` at every sub-solve
       (flexpy does not yet model ``co2_method=total``).
