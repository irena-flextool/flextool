...
def test_warm_chain_equivalence(scen: str, scenario_workdir) -> None:
    ...
    assert list(sols_cold) == list(sols_warm), (
        f"{scen}: warm and cold chains diverge in sub-solve order")
    ...
    for sub_solve in sols_cold:
        ...
        assert rel < 1e-9, (
            f"{scen}/{sub_solve}: warm={w}, cold={c}, rel={rel:.3e}")


# ---------------------------------------------------------------------------
# Test 2: cold-fallback when fingerprint changes between sub-solves.

def test_warm_chain_cold_fallback(scenario_workdir) -> None:
    ...
    for sub_solve, step in sols_warm.items():
        assert step.solution.optimal, (
            f"{sub_solve}: polar_high LP not optimal under warm=True")
...
def test_warm_native_rolling_speedup(scenario_workdir) -> None:
    ...
    """
    work = scenario_workdir("fullYear_roll")
    db_path = work / "tests.sqlite"
...
def test_warm_native_reuses_warmproblem_on_identical_data(
    scenario_workdir,
) -> None:
    ...
    orchestration's ``_PolarHighCascadeSolver.run`` logic in isolation.
    """
