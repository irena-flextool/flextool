"""flexpy ``network_coal_wind_battery_invest_cumulative`` parity —
exercises the cumulative invest cap (``maxCumulative_capacity``) plus
the ``ed_invest_forbidden_no_investment`` pin that the
``_cumulative_invest`` feature pack emits.  Was previously blocked by
...
def test_network_coal_wind_battery_invest_cumulative_parity():
