...
audit and the current flexpy model.py emit it that way.
"""
...
def _sol_value_or_empty(sol, var_name: str) -> Optional[pl.DataFrame]:
    ...
    flexpy's Solution stores variables in a private ``_vars`` dict
...
def commodity_buy_eff_obj(data, sol) -> float:
    ...
    treated as 1.0 (matches flexpy).  See audit §2.2.
    """
...
# ---------------------------------------------------------------------------
# §8 Existing-entity fixed cost (constant) — DEFERRED in flexpy


def existing_entity_fixed_cost_obj(data, sol) -> float:
    """§8.1: + Σ p_entity_all_existing * ed_fixed_cost * inflation_op.

    Currently NOT wired in flexpy (model.py:1149-1161 documents the
