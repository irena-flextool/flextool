...
def test_fast_single_solve_work_base_obj_parity(
...
def test_fast_single_solve_requires_scenario_name(
    tmp_path: Path, scenario_workdir,
) -> None:
...
def test_fast_single_solve_dump_csvs_populates_workdir(
    tmp_path: Path, caplog, scenario_workdir,
) -> None:
...
def test_fast_single_solve_skip_output_emit(
    tmp_path: Path, scenario_workdir,
) -> None:
...
def test_fast_single_solve_p_commodity_price_lh2(
    tmp_path: Path, scenario_workdir,
) -> None:
...
def test_fast_single_solve_p_process_availability_lh2(
    tmp_path: Path, scenario_workdir,
) -> None:
    """Δ.28 — ``p_process_availability`` must populate on the fast path
    for ``lh2_three_region``.
...
def test_fast_single_solve_process_profile_upper_lh2(
    tmp_path: Path, scenario_workdir,
) -> None:
...
def test_fast_single_solve_p_node_capacity_for_scaling_lh2(
    tmp_path: Path, scenario_workdir,
) -> None:
...
@pytest.mark.solver
def test_lh2_three_region_fast_path_obj_parity(
    tmp_path: Path, scenario_workdir,
) -> None:
    ...
    fixture = scenario_workdir("lh2_three_region", db_fixture="lh2")
    db = fixture / "tests.sqlite"
    # ``golden_obj.json`` is committed alongside the disk-fixture data
    # for the LH2 scenario — read it from there as the parity oracle.
    golden = DATA / "work_lh2_three_region" / "golden_obj.json"
