...
_SEED_WORKDIR = _DATA_DIR / "work_base"


@pytest.fixture(scope="function")
def tiny_workdir(tmp_path: Path) -> Path:
    """Copy ``data/work_base`` into ``tmp_path / 'work'`` and return the path.

    ``work_base`` is the smallest end-to-end workdir on disk
    (1 node, 1 process, 1 period, dispatch-only).  Tests layer on
    additional CSVs via :func:`write_csv` (e.g. ``rp_cost_weight.csv``
    overrides) without touching the seed in the repo.
    """
    target = tmp_path / "work"
    shutil.copytree(_SEED_WORKDIR, target)
    return target
