...
def _scenario_for(work_dir: str) -> str:
    """Default convention: scenario name == ``work_dir`` minus the
    ``work_`` prefix.  Most fixtures follow this; multi-solve cascades
    sometimes need a custom mapping (see the orchestration_parity test
    for the override list)."""
    return work_dir.removeprefix("work_")


@pytest.mark.parametrize(
    "work_name,scenario_override",
    [
        ("work_base", "base"),
        (
...
def test_native_cascade_emits_reference_output_raw_files(
    work_name: str, scenario_override: str | None,
) -> None:
    ...
    if not sqlite.exists():
        pytest.skip(f"{work_name} fixture not present")

    ref_dir = work / "output_raw"
    if not ref_dir.exists():
        pytest.skip(f"{work_name} has no reference output_raw/")

    scenario = scenario_override or _scenario_for(work_name)

    # Sanity: scenario actually exists in the DB.
    with api.DatabaseMapping("sqlite:///" + str(sqlite)) as db:
        scenarios = {s.name for s in db.query(db.scenario_sq).all()}
    if scenario not in scenarios:
        pytest.skip(f"scenario {scenario!r} not in {work_name}/tests.sqlite")

    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        with tempfile.TemporaryDirectory(prefix=f"d1_parity_{work_name}_") as tmp:
            wd = Path(tmp)
            ...
            assert not missing, (
                f"{work_name}: native cascade missing {len(missing)} "
            ...
            assert not missing_csv, (
                f"{work_name}: native cascade missing capacity CSVs: "
...
def test_output_writer_state_periods_already_emitted_accumulates(tmp_path) -> None:
