...
# Override scenario name when work_<dirname> doesn't match a scenario.
...
def _discover_fixtures() -> list[tuple[str, str]]:
    """``[(work_dirname, scenario_name), ...]`` for every fixture with a
    ``tests.sqlite`` and ``solve_data/pdtProfile.csv``.
    """
    ...
    for d in sorted(DATA.iterdir()):
        ...
        if not sqlite.exists():
            continue
        # Need pdtProfile.csv as the parity oracle.
        if not (d / "solve_data" / "pdtProfile.csv").exists():
            continue
