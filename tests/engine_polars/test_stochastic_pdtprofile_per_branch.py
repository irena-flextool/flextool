...
flexpy's posture
----------------
flexpy reads ``solve_data/pdtProfile.csv`` directly via its CSV path
...
keys — see progress.md §3.6.1).  So flexpy is immune to the bug at the
...
# Both stochastic fixtures share the same 4-branch pbt_profile structure.
STOCHASTIC_FIXTURES = [
...
@pytest.mark.parametrize("fixture", STOCHASTIC_FIXTURES)
def test_pbt_profile_carries_distinct_branch_values(fixture: str) -> None:
    ...
    assert pbt_path.exists(), f"{fixture}: missing {pbt_path.name}"
    df = pl.read_csv(pbt_path)
    assert df.height > 0, f"{fixture}: pbt_profile.csv is header-only"
    branches = sorted(df["branch"].unique().to_list())
    assert len(branches) >= 2, (
        f"{fixture}: pbt_profile carries only {branches!r} — stochastic "
...
@pytest.mark.parametrize("fixture", STOCHASTIC_FIXTURES)
def test_pdtProfile_per_period_matches_per_branch_pbt(fixture: str) -> None:
    ...
    assert checked > 0, (
        f"{fixture}: no (profile, period, time) entries cross-matched "
        f"between pdtProfile and pbt_profile — fixture/lookup wiring "
        f"broken"
    )
    assert not mismatches, (
        f"{fixture}: pdtProfile drops per-branch pbt_profile values "
