...
- **Use `virtual_unitsize` for unit-commitment fleets with large plant sizes.**  When `startup_method = binary`, the solver commits plants in whole units — if each plant is e.g. 1000 MW, every on/off decision moves 1000 MW of dispatch at once.  Even with tight formulations and modern symmetry detection, the resulting MIP can be **impractically slow to close to a useful tolerance**: a realistic 3-day 3-representative-period UC problem with ~1 GW unit sizes may run up against solver time limits at several percent MIP gap, while the same model with `virtual_unitsize = 200` MW closes to < 0.2 % in about a minute.  Setting `virtual_unitsize` to a smaller value (e.g. 50-200 MW) represents the fleet as more but smaller commitment decisions, which gives the solver finer-grained moves and MIP-practical solve times (the clustered unit-commitment approach — Palmintier & Webster 2012).  The aggregate capacity is unchanged; dispatch results for aggregate fleet output are typically within a few percent of the whole-unit result.  Recommended for any UC model where plant sizes are coarse relative to the demand granularity — which, for realistic power-system studies, is almost always.
