...
Two structural assumptions preserved verbatim from
`flextool/flextoolrunner/blocks.py`:

- **Two-block-deep limit** — one resolution-group block + the natural
  default fine block. Lifting it is a model-design question, not a
  port question.
...
- **3.33.0 (dev)** — `--decomposition lagrangian` rewired onto the
  native polars coordinator (`engine_polars._lagrangian`); the legacy
  GMPL coordinator in `flextool/flextoolrunner/lagrangian.py` is
  deleted.
