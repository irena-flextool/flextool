...
| `FLEXTOOL_FORCE_ROW_SCALING` | unset | `1` / `true` / `yes` / `on` | Test hook that forces `p_use_row_scaling=1` regardless of the per-solve DB value. Used by the Mode B un-scaling benchmark harness. |
