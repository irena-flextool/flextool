...
| `solver`        | Touches a real solver (HiGHS / glpsol), 5–60 s each           |
...
`tests/test_preprocessing_ordered_set_lint.py` AST-walks
`flextool/flextoolrunner/preprocessing/` and fails the build on any
bare `set(…)` call or set literal. The python-preprocessing migration
requires insertion-order-stable containers because preprocessing order
flows through to MPS row/column ordering; the discipline is to use
`dict.fromkeys(...)` or `list` instead. The lint is a no-op until the
