...
- `virtual_unitsize` - [MW] Size of a single unit — used for integer investments (lumped investments) and, in conjunction with `startup_method = binary`, for unit commitment granularity.  If not given, the value of `existing` (capacity) is used, which means every commitment decision moves the full plant — often too coarse for an MIP to solve in reasonable time (see below).
    - FlexTool represents unit commitment with a single general-integer variable per process per timestep, counting how many units are online.  Its upper bound is `capacity / unit_size_for_commitment`, where `unit_size_for_commitment` is `virtual_unitsize` if set, otherwise the physical `existing` capacity.  So setting `virtual_unitsize` to a smaller value than the physical plant size is the same model — the commitment variable just has a larger upper bound and the solver has more granular choices.
    - A plant with `existing = 1000` MW and `virtual_unitsize = 200` MW is literally the same MIP as a plant with `existing = 1000` MW and unit size 200 MW (commitment variable ∈ [0, 5] in both cases).  If you set `virtual_unitsize` equal to the physical unit size, you get the coarse-granularity model and the practical solver-time penalty described in [How to make the Flextool run faster](./how_to.md).  Smaller `virtual_unitsize` values give the solver finer moves, tighter LP relaxations at the root, and MIP-practical solve times at the cost of a slightly different-looking commitment schedule (typically within a few percent of whole-unit dispatch on aggregate).
