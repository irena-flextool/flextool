...
- `solve`: each solve is built from an array of periods (e.g. one period for 2025 and another for 2030). Periods use timesets to connect with a timeline. Parameters are split into two parameter-group tags in the master template — `solve_basics` (knobs a typical user touches) and `solve_advanced` (rolling/nested/stochastic structures, solver options, cross-solve handoff). The split is purely cosmetic in the Spine DB editor; behaviour is identical.
  ...
  - *solver_arguments*: Map of solver option name → value, forwarded verbatim to the active solver. This is where the former `highs_*` shortcuts now live. For HiGHS, set `solver` to choose the method ('simplex' or 'ipm' which is interior point method; use 'choose' for MIP models, since 'simplex' and 'ipm' will not work), `parallel` ('on' or 'off') to control parallelisation of single solves (it can be better to turn it off when executing multiple scenarios in parallel), and `presolve` ('on' or 'off') to toggle presolve (which can have a large impact on solution time when solves are large). These only affect speed, not results.
...
- `energy_margin_method` - How the node's investment-stage energy margin is applied. Options: *none* (default, off, no scaling), *inflow_multiplier* (multiply this node's inflow by `energy_margin_multiplier` in the investment solve only) and *inflow_adder* (add `energy_margin_adder` MWh of extra demand in the investment solve only). See [How to add an energy margin for investment adequacy](how_to.md#how-to-add-an-energy-margin-for-investment-adequacy).
- `energy_margin_multiplier` - [factor] Multiplier applied to the node's inflow in the investment solve only, when `energy_margin_method = inflow_multiplier`. Default 1 (no effect). A value of 1.1 makes the investment stage build for ~10% more demand, so the built capacity meets the true demand under the true (lower) annual VRE availability. Dispatch solves always see the true, un-margined inflow. Constant or period.
- `energy_margin_adder` - [MWh] Extra demand (negative inflow) added to the node in the investment solve only, when `energy_margin_method = inflow_adder`. Default 0 (no effect). Adds a fixed amount of demand rather than scaling existing demand, so it also works on a node with no native inflow (e.g. a zero-inflow slack node) where the multiplier is a no-op. Dispatch solves always see the true, un-margined inflow. Constant or period.
...
- `conversion_flow_coeff` - [factor, default 1] Fuel-equivalent energy per unit of this flow's native units. Multiplied into the flow on both sides of the node-balance equation and `conversion_indirect`: for an indirect-method unit, `Σ v_flow_source × source_flow_coef = Σ v_flow_sink × sink_flow_coef × slope + section × online`, i.e. higher coefficient = more fuel consumed per unit of this edge. Use for fuel-grade energy scaling (e.g. fuel₂ = 2 MWh per flow-unit → `conversion_flow_coeff = 2`), CHP output weighting (a premium electricity output that costs more fuel per unit can have a higher coefficient than a byproduct heat output), or unit-of-measurement conversions. Setting `conversion_flow_coeff = 0` removes the edge from every per-edge capacity / ramp / min-load constraint — the hydro-pass-through pattern. Constant.
- `capacity_max_coeff` - [factor, default 1] Fraction of the unit's capacity available to this edge's upper cap. Applied in `maxFlow`, `maxFromSource`, `maxToSource` (reverse-direction 2-way), `minToSink_1var` (reverse-direction 2-way 1-var cap), and the ramp constraints (both up and down — the ramp limit always scales by the capacity ceiling). Constant.
- `capacity_min_coeff` - [factor, default 1] Fraction of the unit's capacity imposed as a lower cap on this edge when online — combined multiplicatively with the unit-level `min_load`. Applied in `minFlow_minload`, `minFromSource_minload`, and `minToSource` (the ramp constraints use `capacity_max_coeff`, not this, in both directions). Set to 0 to remove the lower cap on this edge (e.g. heat output of an extraction CHP that may fall to zero in pure-condensing mode). Constant.
...
Note that in the case of unit--outputNode the `conversion_flow_coeff` affects *after* the capacity and the other unit constraints — it only shows up in the node balance and in the fuel/efficiency equation. The capacity caps themselves are set by `capacity_max_coeff` and `capacity_min_coeff`.
