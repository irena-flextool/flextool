...
- `capacity_max_coeff` - [factor, default 1] Fraction of the unit's capacity available to this edge's upper cap. Applied in `maxToSink`, `maxFromSource`, `maxToSource` (reverse-direction 2-way), `minToSink_1var` (reverse-direction 2-way 1-var cap), and ramp-up constraints. Constant.
