...
- `capacity_max_coeff` - [factor, default 1] Fraction of the unit's capacity available to this edge's upper cap. Applied in `maxToSink`, `maxFromSource`, `maxToSource` (reverse-direction 2-way), `minToSink_1var` (reverse-direction 2-way 1-var cap), and the ramp constraints (both up and down — the ramp limit always scales by the capacity ceiling). Constant.
- `capacity_min_coeff` - [factor, default 1] Fraction of the unit's capacity imposed as a lower cap on this edge when online — combined multiplicatively with the unit-level `min_load`. Applied in `minToSink_minload`, `minFromSource_minload`, and `minToSource` (the ramp constraints use `capacity_max_coeff`, not this, in both directions). Set to 0 to remove the lower cap on this edge (e.g. heat output of an extraction CHP that may fall to zero in pure-condensing mode). Constant.
