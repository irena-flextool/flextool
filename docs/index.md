...
Finally, the retirements work similar to investments using the same `discount_rate` and `interest_rate` parameters but with `salvage_value` as the benefit from retiring the unit. The current formulation (Dec. 2022) assumes that once an investment has been made, it will remain in the system in perpetuity (unless retired). In other words, **the asset does not disappear from the model at the end of the lifetime automatically**. It will only happen if it is retired. Therefore, the `salvage_value` should not only reflect the actual salvage value, but also the investment cost (so that the model will stop the automatic re-investing). This is bit awkward formulation and is to be improved at a later stage (https://github.com/irena-flextool/flextool/issues/35).
