...
On other Linux architectures or Mac, `highspy` may or may not provide a prebuilt wheel; if it does not, you will need to build HiGHS / highspy from source for your platform. Mac may also require small changes to the code (especially flextoolrunner.py). Spine Toolbox works also on Mac, but apparently has some graphical glitches (21st Sep. 2024 - hopefully will be fixed at some point).
