...
3. **Migrate the canonical example/template databases** — the `templates/` and `how to example databases/` SQLites live in git as JSON (`version/canonical_databases/*.json`). After bumping the schema, round-trip every JSON through the current migration chain:
   ```bash
   python -m flextool.update_flextool.canonical_databases migrate-all
   ```
   This step is idempotent when no schema change is pending. The on-disk `.sqlite` files in `templates/` and `how to example databases/` are recreated on next `update_flextool()` call (or `pytest_configure` for tests) — you don't need to commit them; they're gitignored.

4. **Verify** (optional, also run by CI):
   ```bash
   python -m flextool.update_flextool.sync_master_json_template --verify
   ```

5. **Commit all files together** — the migration code, the regenerated master template, and any canonical-database JSON diffs should land in the same commit or pull request.
...
| `version/canonical_databases/*.json` | Canonical examples + templates — regenerated by `canonical_databases migrate-all` |
