...
def discover_dispatch_entities(
    ...
    Reuses the same dispatch-mapping fields that
    :func:`create_or_update_dispatch_config` reads, classifying each
    discovered name into the entity class the project ``plot_settings.yaml``
    expects:
...
def assign_palette_colors(
    ...
    Mirrors :func:`create_or_update_dispatch_config`'s ``_assign_color`` /
    scenario-color logic: special-token names get their fixed
    ``DEFAULT_SPECIAL_COLORS`` value; everything else cycles the tab20 (or
    tab10 for scenarios) matplotlib palette starting at *start_index* so the
    assigned colors are stable and visually sensible.
