...
#        )
    ...
    def run_model(self):
        ...
        for i, solve in enumerate(all_solves):
            ...
            write_unit_capacity(self.v, self.s, self.p, self.r, solve, 'output_py')
    ...
    def _post_process_results(self):
        ...
        # Create a DataFrame with step_duration values
        dt_dispatch = self.s.dt_realize_dispatch.copy()
        dt_dispatch['step_duration'] = dt_dispatch.apply(
            lambda row: step_duration.loc[(row['period'], row['time'])], axis=1
        )
        
        # Group by period and sum
        hours_in_realized_period = dt_dispatch.groupby('period')['step_duration'].sum()
        # Reindex to match d_realized_period order
        ...
        # Add investments - vectorized using edd_invest triplets
        for _, row in self.s.edd_invest.iterrows():
            e, d_invest, d = row['entity'], row['d_invest'], row['d']
            capacity_add = self.v.invest.loc[d_invest, e] * self.p.entity_unitsize.loc[e, 'value']
            if pd.notna(capacity_add):
                entity_all_capacity.loc[d, e] += capacity_add

        # Subtract divestments - apply to current and all future periods
        for _, row in self.s.ed_divest.iterrows():
            e, d_divest = row['entity'], row['period']
            capacity_sub = self.v.divest.loc[d_divest, e] * self.p.entity_unitsize.loc[e, 'value']
            
            if pd.notna(capacity_sub):
                divest_year = self.p.years_d.loc[d_divest]
                # Subtract from all periods >= divest_year
                for d in self.s.period:
                    if self.p.years_d.loc[d] >= divest_year:
                        entity_all_capacity.loc[d, e] -= capacity_sub
