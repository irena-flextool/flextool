...
Reads raw input through
:class:`flextool.spinedb_backend.SpineDBBackend`, performs the
...
def derive_dc_power_flow(
    ...
    and emits to the Provider via ``provider.put(key, frame)``.  The
    four output frames use the canonical CSV column headers as their
    polars columns so downstream byte-parity is preserved when the
    Provider is snapshotted to disk.
    """
    ...
    # Note: the canonical CSV column is ``process`` (matches the
