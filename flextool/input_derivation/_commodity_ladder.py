...
Ports the two ladder writers from
:mod:`flextool.flextoolrunner.input_writer` into the
:mod:`flextool.input_derivation` tier:

* ``_write_commodity_ladder_cumulative`` (lines 1180-1239)  →
  :func:`derive_commodity_ladder_cumulative`
* ``_write_commodity_ladder_annual`` (lines 1242-1371) →
...
``_PARAMETER_SPECS`` loop in Phase E — with DB fallbacks for setups
that supply periods only through ``solve.period_timeset``).

See ``specs/step_2_5_audit.md`` Section 7 items 11-12 for the
migration plan.
"""
...
# ---------------------------------------------------------------------------
# Helpers (moved verbatim from input_writer.py:1422-1623)
# ---------------------------------------------------------------------------
...
def _collect_periods(backend, provider, api) -> list[str]:
    ...
    ``solve.period_timeset`` — preserving the legacy behaviour of
    :func:`flextool.flextoolrunner.input_writer._collect_periods`.
    """
    periods: list[str] = []
    seen: set[str] = set()
