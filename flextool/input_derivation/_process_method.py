...
Ports the legacy ``_write_process_method`` from
:mod:`flextool.flextoolrunner.input_writer` (lines 971-1138) into the
...
def derive_process_method(
    ...
    *,
    ...
    ct_method_overrides : Mapping[str, str] | None
        ...
        the immediate caller in ``input_writer.write_input`` that
        already holds the dict).

    Mirrors :func:`flextool.flextoolrunner.input_writer._write_process_method`
    (input_writer.py:971-1138) line-for-line.
    """
    # ------------------------------------------------------------------
