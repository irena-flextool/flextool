...
Γ.8.C foundation module.  Direct port of
``flextool/flextoolrunner/stochastic.py`` (394 LOC; the read-only
reference at ``flextool-engine/flextool/flextoolrunner/stochastic.py``).
...
* :func:`write_timeline_matching_map` returns an in-memory dict
  instead of writing CSV (the disk-write was a flextool-side concern;
  see ``audit/solve_orchestration_plan.md §3.5`` for the LOC reduction
  rationale).  Downstream consumers in flextool's ``orchestration.py``
  read the file; flexpy consumes the dict in-memory.
...
* Mutates ``state.timeline.stochastic_timesteps`` (a slight code smell
  preserved from flextool — it would be cleaner to return a fresh
  dict, but downstream readers consume the mutated state).

Behavioural quirks (preserved verbatim from flextool)
-----------------------------------------------------

* **Zero-weight branch**: excluded from ``new_active_time_list``
  (line 311-318 of the reference) but only excluded from
  ``solve_branch__time_branch_lists`` if both ``branch != period`` AND
  the realized flag is not ``"yes"``.  Subtle three-way condition.
...
Reference: ``flextool/flextoolrunner/stochastic.py``.
"""
...
def write_timeline_matching_map(
    ...
    Note (port-time change vs flextool): the flextool reference also
    writes ``solve_data/timeline_matching_map.csv`` to disk.  flexpy
    keeps the result in-memory; the CSV write is a separate concern
    that lives in the orchestrator (Γ.8.D) when file output is
    requested for compatibility with downstream tools.
    """
