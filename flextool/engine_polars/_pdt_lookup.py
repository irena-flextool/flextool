"""Native polars port of ``flextool/flextoolrunner/preprocessing/pd_lookups.py``.

Three lookup classes that replicate the 4-/6-/7-branch GMPL fallback
chains used by ``entity_period_calc_params``:

* :class:`PdLookup`         — 4-branch period-only resolution (mod L1216
  pdNode, L1252 pdProcess).
* :class:`PdtLookup`         — 7- or 9-branch period+time fallback over
  pbt/pd/pt/p + class-level defaults (mod L1176/L1227).
* :class:`PdtLookupPerSide`  — 6-branch period+time fallback for the
  (process, side) per-arc tables (mod L1265/L1279).

Why a class port (not a polars-lazy frame replacement)?
-------------------------------------------------------

Consumers call ``.get(entity, param, period, time)`` per output row in
the inner emission loop.  The lookup itself is essentially a layered
``dict[tuple, float]`` cascade — already optimal for the access pattern.
Restating that as a polars-lazy join chain would be slower at the
small fixture sizes used by these writers and would obscure the
4-branch / 7-branch semantics the legacy code documents in detail.

The CSV-read helpers below mirror the legacy ones byte-for-byte;
each returns a plain dict so the ``.get()`` hot path stays pure
Python (no polars overhead per call).
...
Identical to the legacy class hierarchy.  See module-level docstring
in :mod:`flextool.flextoolrunner.preprocessing.pd_lookups` for the GMPL
provenance.
"""
...
class PdLookup:
    ...
    def __init__(
        ...
        self._branches_for_d = _read_period_branch(period_branch_csv)
