...
# ---------------------------------------------------------------------------
# Minimal output-support CSV writer (fast single-solve path).
# ---------------------------------------------------------------------------
...
def write_output_support_csvs(
    ...
    *,
    ...
    """Emit the tiny subset of ``solve_data/`` CSVs the output writer
    adapter (:mod:`flextool.engine_polars._output_writer`) needs when
    the fast single-solve path skips the full cascade input pipeline.
    ...
    Specifically writes:
      ...
      * ``solve_data/scale_the_objective.csv`` — ``key,value`` form,
        ``value=1.0`` (the LP build does not apply the scale factor,
        so the writer's reciprocal is a no-op).
    ...
    # ── p_step_duration.csv ────────────────────────────────────────────
    # Long format: [solve, period, time, p_step_duration].  One row
    # per (d, t) in flex_data.dt (every (d, t) is realized in the
    # fast single-solve path — no rolling / nested cascade).
