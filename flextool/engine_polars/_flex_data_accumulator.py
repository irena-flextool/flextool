...
_THIN_WRAPPER_BASENAMES: tuple[str, ...] = (
    ...
    # — ramp family (Phase E-b promoted)
    ...
    # post-solve accumulator computed by
    # ``compute_co2_rolling_accumulator`` still flows through the
    # handoff/Provider plumbing when it fires, but single-solve
    # scenarios never run that path — so the basename is conditional,
    # not part of the expected_basenames contract.
    ...
    # ``compute_co2_rolling_accumulator`` (via the handoff plumbing).
    # Listing those basenames in ``expected_basenames`` would re-
    # introduce the contract surface we just retired.
    ...
    # Per-solve block frames populated by emit_block_data into the
    # Provider.
    "entity_block.csv",
