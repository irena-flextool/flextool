"""Inflow-method scaling cascade for ``p_inflow``.

Stochastic + parent-period fold-in (branches 1 and 2 of
``write_pdtNodeInflow``): when the source carries a 3d_map
``node.inflow`` (``pbt_node_inflow``), the per-(n, d, t) Branch-1 fold
(over stochastic nodes' ``solve_branch[d] × first_timesteps[d]``) and
Branch-2 fold (over ``period__branch[d]`` parents) take precedence
over the deterministic Branch 3 additive sum.  See
...
Stochastic branches (``pbt_node_inflow`` fold-in) and parent-period
folds correspond to ``write_pdtNodeInflow`` branches 1 & 2; the helpers
honour those when the source carries a ``pbt_node_inflow`` shape.  For
the default deterministic dispatch fixtures only branch 3 fires.
...
def _inflow_method_lf(source: "InputSource") -> pl.LazyFrame:
    ...
    nodes_lf = nodes.lazy().select(alias_to_axis("name", "n"))
    if raw is None:
