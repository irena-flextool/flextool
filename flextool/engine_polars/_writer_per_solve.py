...
def write_per_solve_sets(solve_data_dir: Path,
    ...
    _write_singles(
        solve_data_dir / "time_in_use_set.csv", "time",
        _project_column(df, 1),
    )
    ...
    _write_singles(
        solve_data_dir / "rp_rep_period_set.csv", "period",
        _project_column(df, 1),
    )
    ...
    # ── dtt ← step_previous (project cols 0, 1, 2) ──────────────────────
    df = _read_csv(
    ...
    for v in pa["period"].to_list() + pfpt["period"].to_list():
        if v and v not in seen_p:
            seen_p[v] = None
    _write_singles(
        solve_data_dir / "period_set.csv", "period",
        list(seen_p.keys()),
    )

    # ── periodAll = period_{group,node,commodity,process,solve} ∪ branch ─
    ...
    for v in _read_csv(solve_data_dir / "branch_set.csv", ["period"], provider=provider)["period"].to_list():
        if v and v not in seen_pa:
            seen_pa[v] = None
    _write_singles(
        solve_data_dir / "periodAll_set.csv", "period",
        list(seen_pa.keys()),
    )

    # ── block = union across (entity_block, process_side_block,
    ...
    _write_singles(
        solve_data_dir / "block_set.csv", "block", list(seen_b.keys()),
    )
        ...
        if s in cur_solve_set:
            for tl in tb_to_tl.get(tb, ()):
                ...
                if key not in pt_seen:
                    pt_seen[key] = None
    _write_tuples(
        solve_data_dir / "period__timeline_set.csv",
    ...
    # d_realize_dispatch_or_invest = d_realized_period ∪
    ...
    for src in (a_df, b_df):
        for d, t in zip(src["period"].to_list(), src["time"].to_list()):
            if d and t and (d, t) not in dtna_seen:
                dtna_seen[(d, t)] = None
    _write_tuples(
        solve_data_dir / "dt_non_anticipativity_set.csv",
    ...
    # Ordered-dedup
    _write_tuples(
        solve_data_dir / "cnd_ladder_set.csv",
    ...
    _write_tuples(
        solve_data_dir / "cndi_ladder_cum_set.csv",
    ...
    _write_tuples(
        solve_data_dir / "cndi_ladder_ann_set.csv",
        ("commodity", "node", "period", "tier"),
        list(dict.fromkeys(ann_rows_out)),
    )

    # cndi_ladder = cum ∪ ann (ordered)
    union4: dict[tuple[str, str, str, str], None] = {}
    for r in cum_rows_out + ann_rows_out:
        if r not in union4:
            union4[r] = None
    _write_tuples(
        solve_data_dir / "cndi_ladder_set.csv",
        ("commodity", "node", "period", "tier"),
        list(union4.keys()),
    )

    # dtdt_next = setof (d_prev, t_prev_solve, d, t) from step_previous
    # Cols: period, time, t_previous, t_previous_within_ts,
    #       period_previous, t_previous_within_solve
    # Project (period_previous=4, t_previous_within_solve=5, period=0, time=1)
    spv_df = _read_csv(
        solve_data_dir / "step_previous.csv",
        ["period", "time", "time_previous", "tprev_within_ts",
         "period_previous", "tprev_within_solve"], provider=provider
    ...
    for r in quads_raw:
        if all(r) and r not in seen_q:
            ...
            quads.append(r)
    _write_tuples(
        solve_data_dir / "dtdt_next_set.csv",
        ("period_prev", "time_prev_solve", "period", "time"), quads,
    )

    # n_fix_storage_{quantity,price,usage}_set ← node column (col 2)
    # from fix_storage_*.csv (period, step, node, value).
    for src, dst in (
    ...
    _write_tuples(
        solve_data_dir / "p_online_dt_set.csv",
...
def write_invest_divest_sets(input_dir: Path, solve_data_dir: Path,
    ...
    _write_tuples(
        solve_data_dir / "ed_divest.csv",
        ("entity", "period"), ed_divest_pairs,
    )
    ed_divest_set = frozenset(ed_divest_pairs)

    # ---- ed_invest_period / ed_divest_period / ed_invest_cumulative ----
    rows = [(e, d) for e, d in ed_invest_pairs
            if methods_for_e.get(e, set()) & _INVEST_PERIOD_METHODS]
    _write_tuples(
        solve_data_dir / "ed_invest_period.csv",
        ("entity", "period"), rows,
    )
    rows = [(e, d) for e, d in ed_invest_pairs
            if methods_for_e.get(e, set()) & _DIVEST_PERIOD_METHODS]
    _write_tuples(
        solve_data_dir / "ed_divest_period.csv",
        ("entity", "period"), rows,
    )
    rows = [(e, d) for e, d in ed_invest_pairs
            if "cumulative_limits" in methods_for_e.get(e, set())]
    _write_tuples(
        solve_data_dir / "ed_invest_cumulative.csv",
        ("entity", "period"), rows,
    )

    # ---- pd_invest / nd_invest / pd_divest / nd_divest -------------
    _write_tuples(
        solve_data_dir / "pd_invest.csv", ("process", "period"),
        [(e, d) for e, d in ed_invest_pairs if e in process_set],
    )
    _write_tuples(
        solve_data_dir / "nd_invest.csv", ("node", "period"),
        [(e, d) for e, d in ed_invest_pairs if e in node_set],
    )
    _write_tuples(
        solve_data_dir / "pd_divest.csv", ("process", "period"),
        [(e, d) for e, d in ed_divest_pairs if e in process_set],
    )
    _write_tuples(
        solve_data_dir / "nd_divest.csv", ("node", "period"),
        [(e, d) for e, d in ed_divest_pairs if e in node_set],
    )

    # ---- edd_history_choice / _automatic / _no_investment ----------
    # For each entity with the relevant lifetime method, walk all
    # (d_history, d_current) pairs and gate on the lifetime window:
    #   * choice / no_investment: p_years_d[d] >= p_years_d[d_h]
    #                              and < p_years_d[d_h] + lifetime
    #   * automatic: p_years_d[d] >= p_years_d[d_h]
    ...
    _write_tuples(
        solve_data_dir / "edd_history_choice.csv",
        ("entity", "period_history", "period"), edd_choice,
    )
    _write_tuples(
        solve_data_dir / "edd_history_automatic.csv",
        ("entity", "period_history", "period"), edd_auto,
    )
    _write_tuples(
        solve_data_dir / "edd_history_no_investment.csv",
    ...
    _write_tuples(
        solve_data_dir / "edd_history_invest.csv",
    ...
    _write_tuples(
        solve_data_dir / "edd_invest.csv",
        ("entity", "period_history", "period"), edd_invest,
    )
    # Use ed_divest_set in a no-op line to satisfy linters — it's
    # consumed in pd_divest/nd_divest filtering above but flake8 doesn't
    # see set-membership reads on the local.
    _ = ed_divest_set  # used above via implicit closure

    # ---- gd_invest / gd_divest / period variants -------------------
    # NB legacy emits gd_divest = gd_invest (mod L1607 oddity preserved).
    ...
    _write_tuples(
        solve_data_dir / "gd_invest.csv",
        ("group", "period"), gd_invest_pairs,
    )
    _write_tuples(
        solve_data_dir / "gd_divest.csv",
        ("group", "period"), gd_invest_pairs[:],
    )
    ...
    _write_tuples(
        solve_data_dir / "gd_invest_period.csv",
        ("group", "period"), rows,
    )
    rows = [
        (g, d) for g, d in gd_invest_pairs
    ...
    _write_tuples(
        solve_data_dir / "gd_divest_period.csv",
        ("group", "period"), rows,
    )
...
def write_ed_invest_forbidden_no_investment(
    ...
    *, provider: "object | None" = None,
    ...
    Emits ed_invest_forbidden_no_investment.csv — the (e, d) pairs in
    ``ed_invest`` where ``(e, 'no_investment')`` is in
    ``entity__lifetime_method`` and the entity's lifetime window
    ...
    for e, d in ed_invest_pairs:
        ...
        if p_years_d.get(d, 0.0) >= s:
            rows.append((e, d))
    _write_tuples(
        solve_data_dir / "ed_invest_forbidden_no_investment.csv",
        ("entity", "period"), rows,
    )
