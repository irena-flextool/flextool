"""Per-solve scaling analyzer — polars / in-memory.

Implements a lightweight numerical diagnostic that consumes an
in-memory :class:`~flextool.engine_polars.input.FlexData` bag.

The public API is :func:`analyze_solve`.  See :class:`ScaleTable`,
:class:`FamilyStats`, the JSON helpers, and the module-level
:data:`_scale_cache` for the supporting machinery.
"""
...
#     so we deliberately do NOT also set ``threads=1`` here.  The user
#     can override via ``--highs-threads N`` (N > 1), which flips
#     ``parallel`` to ``on`` and sets ``threads=N`` in
#     ``_finalise_highs_options`` — N == 1 leaves this pin intact and
#     keeps default behaviour byte-identical.  HiGHS still initialises a
#     single, process-global thread scheduler on the FIRST
#     ``Highs::run()`` of the process; subsequent ``Highs`` instances
#     that try to set ``threads`` to a different value are rejected with
#     ``"global scheduler has already been initialized"``.  We avoid
#     that error path because ``_finalise_highs_options`` resolves
#     ``FLEXTOOL_HIGHS_THREADS`` from a process-level env var set by the
#     CLI before any solve runs — every sub-solve in the cascade sees
#     the same value, so the scheduler is initialised once with the
#     final thread count and never reconfigured.  ``parallel=off`` (the
#     default) is sufficient for determinism (no concurrent simplex
#     trajectories); users who opt in to N > 1 explicitly accept the
#     determinism/wall-clock trade-off.
...
def _estimate_rough_obj_inmemory(
    ...
    Pulls VOM / CAPEX sums directly from the in-memory Params.
...
# infeasibility on the user-scaled model.
