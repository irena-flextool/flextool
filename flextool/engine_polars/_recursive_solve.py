...
* This is the algorithmically most-complex orchestration module.  It
  ...
  chains.  Every orchestration phase downstream reads the flat
  ``all_solves`` list and per-solve time lists this module produces;
  any divergence manifests as wrong solve ordering, missing rolls, or
  mis-scoped active times.

* :class:`RecursiveSolveBuilder` mutates several fields of
  ``state.solve``:

  - ``roll_counter[solve] += 1`` per roll (R-O5).  Reset at the start
    of every top-level ``define_solve_recursive`` call so repeated
    invocations on the same SolveConfig don't desync.
...
* **"More than one solve in a rolling solve, not managed"** raise:
  the fix-storage handoff between sibling rolling children of a
  rolling parent isn't implemented downstream.
"""
