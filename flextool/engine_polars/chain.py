...
class ChainStep:
    ...
    handoff : flextool.SolveHandoff
        The handoff carriers derived from the polar_high solution
        (see :func:`flextool.input.build_handoff_from_solution`).
    warm_used : bool
...
def run_chain(
    ...
    *,
    ...
    2. Resolves the scenario name from the dir-name convention (see
       ...
       ``scenario`` kwarg / ``FLEXTOOL_NATIVE_SCENARIO`` env var.
    3. Delegates to
    ...
    scenario : str | None, default None
        ...
        ``FLEXTOOL_NATIVE_SCENARIO`` env var, then falls back to the
    ...
    if scenario is None:
        scenario = os.environ.get("FLEXTOOL_NATIVE_SCENARIO") or None
    if scenario is None:
    ...
    if scenario is None:
        raise ValueError(
            ...
            f"FLEXTOOL_NATIVE_SCENARIO, or rename the work directory to "
