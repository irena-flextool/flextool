"""Agent 10: user-facing scaling diagnostic report — polars / in-memory port.
...
6. Near-duplicate parameter clusters (skipped in this port — no CSV dir
   available; section is omitted gracefully).
...
Port differences from :mod:`flextool.flextoolrunner.scaling_report`:

* ``input_dir`` (CSV directory) replaced by ``flex_data`` (FlexData bag).
* ``highs_log_path`` replaced by ``solution`` (polar_high.Solution object).
* Composite-mismatch detector reads topology from ``flex_data`` DataFrames.
* HiGHS matrix ranges extracted directly from ``solution.highs.getLp()``
  rather than parsed from a log file.
* Near-duplicate section is skipped (no CSV scan available in this path).
"""
...
def find_composite_mismatches(
    ...
    Mirrors :func:`flextool.flextoolrunner.scaling_report.find_composite_mismatches`
    but reads topology and unitsizes from *flex_data* instead of CSV files.
