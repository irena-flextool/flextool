"""Surgical source-only FlexData loader.

Single-solve fast path: reads directly from a
:class:`flextool.engine_polars._spinedb_reader.SpineDbReader` and
constructs a :class:`flextool.engine_polars.input.FlexData` without
touching the workdir CSVs.
...
The slow path (``run_chain_from_db`` → ``_native_input_writer``)
...
def load_flextool_source_only(
    ...
    *,
    ...
    # orchestration loop and the inner ``_PolarHighCascadeSolver`` wrapper,
