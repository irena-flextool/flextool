...
def load_data(
    ...
    *,
    ...
    frames are produced by
    :mod:`flextool.input_derivation._commodity_ladder_sets`.
    """
    inp = Path(inp_dir)
    sd = Path(sd_dir)
    ...
    if fdk is not None and fdk.height > 0:
        ...
        # ``_emit_period_calc._keyed_frame``); cast to Float64 to match
    ...
    # Phase 4.1a / 4.2-1g — the cross-roll cumulative-realized MWh
    # accumulator rides on ``handoff/cumulative_commodity`` (written by
    # the iteration-start translator from the prior solve's SolveHandoff).
...
def add_constraints(
    ...
    # expression and dropping the price coefficient.  Here we have
    # three index frames:
