...
class CalibConfig:
    ...
    ``over_build_tightness``— over-build-guard tightness (consumed by C1c).
...
class IterRecord:
    ...
    trajectory pairs each observation with the input that produced it.
    """
...
def compute_step(
    ...
    **C1b — sizing.**  Each shedding node's residual unserved energy is
    converted into the CONSTANT per-timestep ``energy_margin_adder``
    increment that (undamped) would inject that residual annual MWh back as
    demand: ``increment(node) = λ · residual(node) / W`` (see
    :func:`flextool.calibrate._sizing.sized_increments`).  ``W`` is the
    invest-timeline annualisation weight, computed ONCE by the loop and
    threaded in.
    ...
    **C1c — over-build guard.**  On top of sizing this step:
    ...
    2. from the SECOND correction onward (``prev_record is not None``) runs
       ...
       node whose adequacy has stalled while only spill grows — the
       three-condition rule keyed off *curtailment* (this iteration's
       per-sink spill), *prev_record* (the prior iteration's residual and
       curtailment) and *baseline_curtailment* (iteration-0 spill, so only
       margin-INDUCED curtailment counts).  The guard freezes only nodes that
       were SHEDDING last round (``prev_record.residual > shed_tol``, the same
       tolerance the sizer used above) — a node that only STARTS shedding this
       round was never bumped, so it gets its first bump rather than being
       frozen.  On the first correction there is no prior to diff, so nothing
       is flagged and every shedding node is bumped.
    ...
    increments = sized_increments(residual, W=W, lam=lam)
    ...
    return guard_freeze(
        ...
        prev_curtailment=prev_record.curtailment,
        baseline_curtailment=baseline_curtailment,
        tightness=config.over_build_tightness,
...
def run_calibration(
    ...
    # C1c over-build guard state: the iteration-0 curtailment baseline (only
    # margin-INDUCED spill above it can flag a node) and the PERSISTENT set of
    # nodes frozen as resource-capped (never bumped again once flagged).
    baseline_curtailment: dict[str, float] = {}
    flagged: set[str] = set()
    converged = False
    ...
    for k in range(config.iterations + 1):
        ...
        # Capture the iteration-0 curtailment as the guard's baseline (only
        # spill that rises ABOVE this counts as margin-induced over-build).
        if k == 0:
            baseline_curtailment = dict(curtailment)
        ...
        if k < config.iterations:
            increments, newly_flagged = compute_step(
                residual, curtailment, penalty_by_node, prev_record, config,
                W=W, flagged=flagged, baseline_curtailment=baseline_curtailment,
            )
            ...
            for node, inc in increments.items():
                adders[node] = adders.get(node, 0.0) + inc
            prev_record = record
