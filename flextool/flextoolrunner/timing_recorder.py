...
Replaces the legacy hand-rolled appends to two ``solve_progress.csv``
files (one in ``solve_data/`` written by five different writers, one in
``output/`` written by ``log_time``) with a single structured CSV at
