...
Today, solve-to-solve state moves through ``solve_data/*.csv`` files
(plus ``shutil.copy`` shuffles in ``orchestration.py``).  ``SolveHandoff``
is the in-memory replacement: each carrier kind (realized invest,
storage state quota, …) becomes a polars DataFrame on the dataclass,
and the orchestration loop's post-solve hook deposits one entry per
completed solve into ``RunnerState.handoffs``.

This module is **add-on only** — when ``RunnerState.handoffs is None``
(the default), nothing in the existing flextool pipeline behaves
differently.  Hooks check the slot; absent → fall through to the
file-based path.
...
def capture_post_solve(state, solve_name: str) -> None:
    ...
    Called from ``orchestration.py`` immediately after ``solver.run()``
    returns 0, but only when ``state.handoffs is not None`` (the opt-in
    flag).  The capture is **additive** — existing post-solve CSV
    writes (e.g. ``p_entity_period_existing_capacity.csv``) continue
    unchanged, so file-based downstream consumers see the same bytes
    they always did.  This hook merely records the same data in
    memory for future in-memory consumers.
