...
def write_process_source_sink_ramp_unions(
    ...
    """flextool.mod — process_source_sink_ramp (5-way union of the
    ramp_limit_*/ramp_cost sets).

    The 5 input ramp_*.csv files are written by
    write_process_source_sink_ramp_family above; this just unions them.

    Earlier versions of this writer also produced 4 dtttdt-filtered
    ramp_limit_*_{up,down} variants (dimen-9 sets). Those were removed
    in batch 68 — they were never referenced by any constraint
    (the ramp_*_constraint families at mod L3468-3543 inline the same
    filter on block_dtttdt directly).
    """
    ramp_files = (
