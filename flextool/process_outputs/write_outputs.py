...
def _read_outputs(
    ...
    *,
    ...
    ``flex_data`` + ``solution`` are required for the in-memory
    parameter / set reading.  Variables (parquets) are read from
    ``output_raw/`` where the polars-LP write path emits them.

    ``solve_steps`` (multi-solve): an ordered list of
    ...
    if flex_data is None or solution is None:
        raise ValueError(
            "_read_outputs requires flex_data and solution; "
...
def log_time(log_string, start, timing_recorder=None):
    ...
    Feeds the ``TimingRecorder`` when supplied.  The ``print(...)`` line
    is kept for human-readable stdout — that's the file most operators
    eyeball.
    """
...
def _resolve_settings(write_methods, output_config_path, active_configs, plot_rows,
    ...
    ``output-spinedb`` from the settings DB is honored on BOTH the native
    solve path and the parquet-replay path: a True flag writes the results
    SpineDB either way (the run produces it from the live solve; re-create
    rebuilds/augments it from parquet without re-solving).
    """
    db_reachable = False
    if settings_db_url:
...
def write_outputs(scenario_name, output_config_path=None, active_configs=None, output_funcs=None, output_location=None, subdir=None, read_parquet_dir=False, write_methods=None, plot_rows=None, debug=False, single_result=None, settings_db_url=None, fallback_output_location=None, plot_file_format=None, raw_output_dir=None, only_first_file=False, timing_recorder=None, flex_data=None, solution=None, solve_name=None, solve_steps=None, flex_data_provider=None, results_db_url=None):
    ...
    Args:
        ...
        flex_data: FlexData (polars input bundle) — required for the
            in-memory parameter / set reading.  Pass-through from the
            orchestration step's last solve.
        solution: polar_high.Solution — required; used by the in-memory
            read_parameters for entity_all_capacity (and friends)
            derivation.
        ...
        results_db_url: optional target SpineDB URL for the ``spinedb``
            ...
            write-method runs on BOTH paths and ``output-spinedb`` from the
            settings DB is honored on both: on the native solve path it uses
            the live ``s`` / ``par`` namespaces, while on the
            ``read_parquet_dir`` REPLAY path it rebuilds the ``s`` shim from
            the processed parquet (``build_replay_s``, with ``par=None`` —
            the two discount/inflation params are dropped on the replay path).
    """
            ...
            for table_name, df in results.items():
                ...
                if 'solve' not in df.index.names and 'period' in df.index.names:
                    if 'time' in df.index.names:
                        ...
                        # Key off the named 'period'/'time' levels, not positional
                        # index slots — entity-first indexes (e.g. (unit, period, time))
                        # otherwise read the wrong levels and miss every lookup.
                        period_level = df.index.get_level_values('period')
                        time_level = df.index.get_level_values('time')
                        solve_vals = [pt_to_solve.get((p, t), '') for p, t in zip(period_level, time_level)]
